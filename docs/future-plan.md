...
3. **Complete logistics fulfillment:** courier approval/profile lifecycle, courier assignment, pickup/hub scans, sort and dispatch operations, linehaul/hub routing, delivery exceptions, returns, and waybill/label ownership.
...
The mobile repository is future work and is not part of the current Laravel implementation. When both repositories are ready:

- Agree on a stable API contract before connecting screens. Add a versioned `/api/v1` surface (or an equivalent gateway) with documented JSON schemas and error responses.
