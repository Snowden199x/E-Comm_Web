    ...
    <div class="p-4 sm:p-5 lg:p-6" x-data="{ confirmAction: null }">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl p-5 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900">{{ $admin->created_at->format('M d, Y') }}</p>
                    </div>
                    <div>
        ...
        <div x-show="confirmAction === 'suspend'" x-cloak
            ...
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm" @click.stop>
                <h3 class="font-bold text-lg text-gray-900 mb-2">Suspend this admin?</h3>
                <p class="text-sm text-gray-500 mb-4">They will temporarily lose access to the Admin Panel until
                    reactivated.</p>
                <div class="flex gap-3">
                    <button type="button" @click="confirmAction = null"
                        class="flex-1 border border-gray-300 text-gray-700 text-sm font-medium py-2 rounded-lg hover:bg-gray-50">Cancel</button>
                    <form method="POST" action="{{ route('admin.account-management.suspend', $admin) }}"
                        class="flex-1">
                        @csrf
                        <button type="submit"
                            class="w-full bg-yellow-500 text-white text-sm font-medium py-2 rounded-lg hover:bg-yellow-600">Suspend</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div x-show="confirmAction === 'deactivate'" x-cloak
            ...
            @click.self="confirmAction = null">
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm" @click.stop>
                <h3 class="font-bold text-lg text-gray-900 mb-2">Deactivate this admin?</h3>
                <p class="text-sm text-gray-500 mb-4">This permanently disables their access. This action should only be
                    used for accounts no longer in use.</p>
                <div class="flex gap-3">
                    <button type="button" @click="confirmAction = null"
                        class="flex-1 border border-gray-300 text-gray-700 text-sm font-medium py-2 rounded-lg hover:bg-gray-50">Cancel</button>
                    <form method="POST" action="{{ route('admin.account-management.deactivate', $admin) }}"
                        ...
                        @csrf
                        <button type="submit"
                            class="w-full bg-red-600 text-white text-sm font-medium py-2 rounded-lg hover:bg-red-700">Deactivate</button>
                    </form>
                </div>
            </div>
        </div>
        @if (session('error'))
            <div class="mb-4 bg-red-100 text-red-700 text-sm font-medium px-4 py-3 rounded-lg">
                {{ session('error') }}
            </div>
        @endif
    </div>
