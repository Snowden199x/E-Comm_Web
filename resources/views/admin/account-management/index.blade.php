<x-admin-layout>
    <div class="p-4 sm:p-5 lg:p-6" x-data="{ tab: '{{ $errors->any() && old('email_username') !== null ? 'admin-accounts' : 'my-account' }}', showCreateModal: {{ $errors->any() && old('email_username') !== null ? 'true' : 'false' }}, editProfile: false, changePass: false, actionId: null }">
        @if (session('generated_password'))
            <div x-data="{ show: true }" x-show="show" x-cloak
                ...
                <div class="bg-white rounded-2xl p-6 w-full max-w-md">
                    ...
                    <p class="text-sm text-gray-500 mb-4">Copy these now and give to the admin. This is shown only once
                        unless viewed again from their profile before they set their own password.</p>
                    <div class="bg-gray-50 rounded-lg p-4 space-y-2 mb-4">
                        <p class="text-sm text-gray-700">Email: <span
                                class="font-mono">{{ session('generated_email') }}</span></p>
                        <p class="text-sm text-gray-700">Temporary Password: <span
                                class="font-mono font-bold">{{ session('generated_password') }}</span></p>
                    </div>
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-500">Manage your administrator profile, security settings, and authorized admin
                accounts.
            </p>
        </div>

        @if (session('confirmation'))
            <div class="mb-4 bg-green-100 text-green-700 text-sm font-medium px-4 py-3 rounded-lg">
                @php
                    $messages = [
        ...
        <div class="flex items-center gap-2 mb-6 bg-white rounded-xl p-1 w-fit shadow-sm">
            <button type="button" @click="tab = 'my-account'"
                ...
                class="px-4 py-2 rounded-lg text-sm font-medium transition">My Account</button>
            @if ($currentAdmin->is_super_admin)
                <button type="button" @click="tab = 'admin-accounts'"
                    :class="tab === 'admin-accounts' ? 'bg-[#3b1735] text-white' : 'text-gray-600'"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition">Admin Accounts</button>
            @endif
        </div>
        ...
        <div x-show="tab === 'my-account'">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm flex-1">
                            <div>
                                ...
                                <p class="font-medium text-gray-900">{{ $currentAdmin->first_name ?? '—' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-400">Last Name</p>
                                <p class="font-medium text-gray-900">{{ $currentAdmin->last_name ?? '—' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-400">Middle Initial</p>
                                <p class="font-medium text-gray-900">{{ $currentAdmin->middle_initial ?? '—' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-400">Contact Number</p>
                                <p class="font-medium text-gray-900">{{ $currentAdmin->phone_number ?? '—' }}</p>
                            </div>
                            <div>
                                <p class="text-gray-400">Primary Email</p>
                                <p class="font-medium text-gray-900">{{ $currentAdmin->email }}</p>
                            </div>
                            <div>
                                <p class="text-gray-400">Secondary Email</p>
                                <p class="font-medium text-gray-900">{{ $currentAdmin->recovery_email ?? '—' }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Login & Security -->
                <div class="bg-white rounded-2xl p-5 shadow-sm">
                    <h3 class="font-bold text-gray-900 mb-4">Login & Security</h3>
                    ...
                    <div class="p-3 border border-gray-100 rounded-lg">
                        <p class="text-sm font-medium text-gray-900 mb-1">Last Login</p>
                        <p class="text-xs text-gray-500">
                            {{ $currentAdmin->last_login_at ? $currentAdmin->last_login_at->format('M d, Y \a\t g:i A') : 'No record yet' }}
                        </p>
                    </div>
                </div>
        ...
        @if ($currentAdmin->is_super_admin)
            <div x-show="tab === 'admin-accounts'" x-cloak>
                <div class="bg-white rounded-2xl p-5 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h3 class="font-bold text-gray-900">Administrator Accounts</h3>
                            <p class="text-sm text-gray-500">Manage authorized administrators who can access the Vendo
                                Admin Panel.</p>
                        </div>
                        ...
                        search() {
                            ...
                            this.timer = setTimeout(() => {
                                ...
                                fetch('{{ route('admin.account-management.table') }}?' + params)
                                    .then(r => r.text()).then(html => { document.getElementById('admins-table-wrap').innerHTML = html; });
                            }, 250);
                        }
                    ...
                    <div id="admins-table-wrap" x-data="{
                        poll() {
                            fetch('{{ route('admin.account-management.table') }}?' + new URLSearchParams({ search: '', status: 'all' }))
                                .then(r => r.text()).then(html => { document.getElementById('admins-table-wrap').innerHTML = html; });
                        }
            ...
            <div x-show="showCreateModal" x-cloak
                ...
                <div class="bg-white rounded-2xl p-6 w-full max-w-lg" @click.stop>
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-bold text-lg text-gray-900">Create Administrator</h3>
                    ...
                    <form method="POST" action="{{ route('admin.account-management.store') }}">
                        ...
                        <p class="text-xs text-gray-500 mb-4">A temporary password will be generated automatically.
                            You'll see it once after creating the account — copy it to give to the admin. They'll be
                            required to set their own password on first login.</p>
                        <button type="submit"
                            class="w-full bg-[#3b1735] text-white text-sm font-semibold py-2.5 rounded-lg hover:bg-[#4d1f45]">
                            Create Account
                        </button>
        ...
        <div x-show="editProfile" x-cloak class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <div class="bg-white rounded-2xl p-6 w-full max-w-lg" @click.stop>
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-lg text-gray-900">Edit Profile</h3>
        ...
        <div x-show="changePass" x-cloak class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <div class="bg-white rounded-2xl p-6 w-full max-w-md" @click.stop>
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-lg text-gray-900">Change Password</h3>
